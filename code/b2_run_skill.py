...
from skills import skill_error_payload  # advanced4: 在将 agent/ 加入 sys.path 后导入。
...
SKILL_MODULES = {
    ...
    "read_and_convert": "skills.read_and_convert",  # advanced3: 固定复合 Skill，file_reader -> format_converter。
    "search_and_read": "skills.search_and_read",  # advanced3: 固定复合 Skill，local_file_search -> file_reader。
    "analyze_and_convert_table": "skills.analyze_and_convert_table",  # advanced3: 固定复合 Skill，table_analyzer -> format_converter。
}
...
def run_skill(skill_name: str, input_data: dict, data_root: str | None = None, output_dir: str | None = None) -> dict:
    ...
    try:
        ...
        status = "success"
        error = None
    except Exception as exc:  # advanced4: 将 Skill 异常转换为结构化业务结果。
        output = None
        status = "error"
        error = skill_error_payload(exc)  # advanced4: 在 B2 SkillResult 中返回稳定的 code/category/retryable 字段。
        ...
        append_jsonl(
            {
                ...
                "error_code": result["error"].get("code") if result.get("error") else None,  # advanced4: 使错误日志可按错误码筛选。
